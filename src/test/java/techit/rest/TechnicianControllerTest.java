...
class TechnicianControllerTest extends AbstractTransactionalTestNGSpringContextTests {
	...
	void getTickets1() throws Exception {
		this.mockMvc.perform(get("/technicians/4/tickets").header("Authorization", "Bearer " + bleeUserToken)).andExpect(status().isOk())
				.andExpect(jsonPath("$[0].id").value(2));
	}

	@Test
	void getTickets2() throws Exception {
		this.mockMvc.perform(get("/technicians/3/tickets").header("Authorization", "Bearer " + bleeUserToken)).andExpect(status().is(403))
